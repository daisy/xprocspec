...
If you wish to try it out; run the XProc script `xprocspec.xpl` with `test/basic.xprocspec` as input on the source port.
If you're on Windows you should also provide the `temp-dir` option which should be a `file:///` URI to a directory on your disk
(it defaults to `file:/tmp/` which should work in both Linux and OSX).
On the `html` output port you will get a HTML report which you can open in your browser to view the results of the test.
The `junit` output port contains a JUnit report used the [pipeline-xproc-connector](https://github.com/bertfrees/pipeline-xproc-connector) project.
...
### Contents:
* x:document: Optional XPath context document for the select expression.
